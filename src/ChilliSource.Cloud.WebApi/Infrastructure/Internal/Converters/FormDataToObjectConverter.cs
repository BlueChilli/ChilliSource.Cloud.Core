...
namespace ChilliSource.Cloud.WebApi.Infrastructure.Internal.Converters
{
    internal class FormDataToObjectConverter
    {
        ...
        private bool TryGetFromFormData(Type destinitionType, string propertyName, out object propValue)
        {
            ...
            if (destinitionType == typeof(HttpPostedFileBase) || destinitionType == typeof(HttpFile))
            {
                HttpPostedFileBase HttpPostedFileBase;
                if (SourceData.TryGetValue(propertyName, out HttpPostedFileBase))
                {
                    propValue = HttpPostedFileBase;
        ...
        {
            if (type == typeof(HttpPostedFileBase) || type == typeof(HttpFile))
                return true;
