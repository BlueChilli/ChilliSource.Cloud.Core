...
namespace ChilliSource.Cloud
{
    ...
    internal interface IPrimaryKeysMetadata<T>
    {
        ...
        Expression<Func<T, bool>> GetKeysFilter(object[] keyValues);
    }
        ...
        where TEntity : class
    {
        ...
        private static IList<EdmMember> GetPrimaryKeysDefinition(DbContext context)
        {
            ...
            var metadataWs = ((IObjectContextAdapter)context).ObjectContext.MetadataWorkspace;
            return metadataWs.GetItems<EntityType>(DataSpace.OSpace).Where(item => item.FullName == typeFullName)
                        .Take(1)
        ...
        public Expression<Func<TEntity, bool>> GetKeysFilter(object[] keyValues)
        {
            if (keyValues == null || keyValues.Length == 0)
                return (TEntity e) => false;

            if (keyValues.Length != _expressions.Length)
                throw new ApplicationException("Wrong number of key values.");
